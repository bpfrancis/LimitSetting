...
//For the temperature plots
  ...
  while(1){
    ...
    int mStop, mBino;
    double acc, xsec, xsecError, obsLimit, expLimit, exp_m1s, exp_m2s, exp_p1s, exp_p2s;
    fin >> mStop >> mBino >> acc >> xsec >> xsecError >> obsLimit >> expLimit >> exp_m1s >> exp_m2s >> exp_p1s >> exp_p2s;
    if(!fin.good()) break;
